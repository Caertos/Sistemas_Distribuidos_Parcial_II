...
echo -e "  👨‍⚕️ Cardiólogo:  ${GREEN}cardiologo1${NC} / ${GREEN}secret${NC} - Dr. Juan Cardiólogo"
echo -e "  👩‍⚕️ Enfermera:    ${GREEN}enfermera1${NC} / ${GREEN}secret${NC} - Enf. Laura Admisión"
...
echo -e "  ${CYAN}./k8s/populate_db_k8s.sh${NC} - Poblará la BD con más usuarios y datos de ejemplo"
echo ""
...
if curl -s --connect-timeout 3 "http://$FINAL_MINIKUBE_IP:$FINAL_NODEPORT/health" | grep -q "healthy" 2>/dev/null; then
    ...
    if [[ $POPULATE_DB =~ ^[Yy]$ ]]; then
        ...
        echo -e "  👩‍🦰 Ana García (paciente1/secret) - Insuficiencia cardíaca, medicamentos, alergias"
        echo -e "  👨‍⚕️ Dr. Juan Cardiólogo (cardiologo1/secret) - Especialista tratante"
        echo -e "  💉 Enf. Laura (enfermera1/secret) - Personal de admisión y triage"
        echo -e "  👤 Dr. Carlos Administrador (admin1/secret) - Acceso completo al sistema"
        echo -e "  👁️ Lic. María Auditora (auditor1/secret) - Revisión y auditoría"
    fi
else
