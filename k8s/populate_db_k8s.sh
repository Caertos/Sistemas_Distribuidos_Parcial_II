...
execute_sql() {
    local sql="$1"
    kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -c "$sql"
}

# Función para verificar conexión a la base de datos
check_db_connection() {
    log "Verificando conexión a la base de datos (Kubernetes)..."
    if ! kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" > /dev/null 2>&1; then
...
check_existing_data() {
    ...
    local user_count=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM users;" | tr -d ' ')
    local patient_count=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM paciente;" | tr -d ' ')
...
show_summary() {
    ...
    local users=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM users;" | tr -d ' ')
    local patients=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM paciente;" | tr -d ' ')
    local doctors=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM profesional;" | tr -d ' ')
    local conditions=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM condicion;" | tr -d ' ')
    local medications=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM medicamento;" | tr -d ' ')
    local encounters=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM encuentro;" | tr -d ' ')
    local observations=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM observacion;" | tr -d ' ')
    local vital_signs=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM signos_vitales;" | tr -d ' ')
    local allergies=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM alergia_intolerancia;" | tr -d ' ')
    local appointments=$(kubectl exec citus-coordinator-0 -- psql -U "$DB_USER" -d "$DB_NAME" -t -c "SELECT COUNT(*) FROM cita;" | tr -d ' ')
